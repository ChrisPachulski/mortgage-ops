...
| APR-04 | Phase 7 | Done partial (07-07 — 20 oracle fixtures shipped at tests/fixtures/apr/oracle/ffiec_001..020.json via autonomous-execution override accepted by project owner; mirrors Phase 5 ARM-06 partial-closure precedent. Provenance distribution: 12/20 class 'engine-emitted, cross-validated against Wikipedia worked example' (regular-monthly archetypes ffiec_001..012 — engine APR == nominal rate exactly via PV-form-collapse algebraic identity against Phase 1 \$200k @ 6.5%/30yr → \$1,264.14 oracle anchor); 8/20 class 'engine-emitted' only (odd-first-period archetypes ffiec_013..016 with 5/10/15/20-day odd FPs + finance-charge archetypes ffiec_017..020 — pending future HMDA Platform Docker / FFIEC APRWIN-under-Wine / CFPB Rate Spread Calculator cross-validation per oracle/README.md §Recommended Path to Full Closure). scripts/_generate_apr_oracle_fixtures.py (516 lines) is the deterministic regenerator + single source of truth; tests/fixtures/apr/oracle/README.md (234 lines) documents the 5-step fallback oracle chain + per-fixture provenance table + fallback substitution log + recommended path to full closure. test_apr_ffiec_oracle_fixtures_match_within_decimal_00001 flipped from Wave-0 single-stub xfail to @pytest.mark.parametrize over 20 stems asserting \|engine - expected\| <= Decimal('0.00001') + iterations <= iterations_max + iterations <= 50 — all 20 cases PASS. 13 of 13 Wave-0 stubs in tests/test_apr.py now flipped (zero APR-XX xfails remain). 502 passed + 4 skipped + 1 xfailed [was 482/4/2; +20 net pass / -1 xfail; remaining xfail is Phase 5 ARM oracle deferral, NOT Phase 7]. PHASE 7 COMPLETE 8/8 plans; APR-01..APR-08 all closed; ROADMAP SC-1..SC-5 all closed) |
...
| STRS-01 | Phase 8 | Done (08-02 — lib.stress.rate_shock(loan, rates, baseline_label) per-cell loop over Phase 3 lib.amortize.build_schedule with monthly_pi/total_interest capture + delta_vs_baseline_monthly + delta_vs_baseline_pct + RATE_SHOCK_MONOTONE_PI invariant check per 08-RESEARCH §6.4; smoke: $400k/30yr at {0.06, 0.065, 0.07} returns [2398.20, 2528.27, 2661.21] pinning the CONVENTIONS.md Phase 3 oracle 2528.27 verbatim; D-02-02 loan.model_copy(update={'annual_rate': rate}) idiom verified vs Phase 4 reverse-mode flow; test_rate_shock_per_cell_calls_phase3_engine_exact_to_cent flipped from Wave-0 strict-xfail to PASS with Phase 3 oracle anchor at rows[1]) |
| STRS-02 | Phase 8 | Done (08-02 — lib.stress.income_shock(base_request, reductions, dti_threshold) per-cell loop applying per-applicant gross_monthly_income scaling by (1-reduction) per D-02-03 + Phase 4 D-06 sum aggregation, re-running lib.affordability.evaluate(shocked_request), capturing dti_back/breaches_threshold/blocked_by; smoke: $10k single_applicant baseline AffordabilityRequest at 0% reduction matches baseline dti_back exactly + 50% reduction roughly doubles dti_back AND breaches 0.43 threshold; D-02-06 model_copy on AffordabilityRequest preserves discriminated-union runtime type; test_income_shock_per_cell_calls_phase4_engine_with_threshold_breach flipped from Wave-0 strict-xfail to PASS) |
| STRS-03 | Phase 8 | Done (08-02 — lib.stress.arm_path(base_arm_request, paths) + lib.stress._synthesize_index_path covering closed-set RatePath dispatch per D-01-05 + D-02-04 [parallel-shift / gradual-rise / fall-then-rise] generating one IndexPathEntry per reset trigger via the promoted lib.arm.compute_reset_triggers; arm_path per-cell loop calling lib.arm.build_arm_schedule(syn) where syn = base_arm_request.model_copy(update={'index_path': index_path}); smoke: 5/1 ARM 30yr base + 3 canonical paths produce positive total_interest with parallel-shift > fall-then-rise sanity dominance + reset_count==25 across all 3 paths for the [61,73,...,349] trigger sequence; lib.arm._compute_reset_triggers promoted to public lib.arm.compute_reset_triggers via D-02-01 single-line rename + backward-compat alias mirroring Phase 5 D-14 quantize_rate promotion; Phase 5 baseline preserved verbatim; test_arm_path_three_canonical_paths_total_interest + test_arm_path_30yr_horizon_reset_count flipped from Wave-0 strict-xfail to PASS) |
| STRS-04 | Phase 8 | Complete |
| PNTS-01 | Phase 8 | Done (08-03 — lib.points.simple_breakeven(points_cost, monthly_savings) Decimal-safe ceil(points_cost / monthly_savings) via to_integral_value(rounding=ROUND_CEILING) under localcontext(MONEY_CONTEXT); returns int|None where None signals monthly_savings <= 0 per D-03-01 mirroring Phase 4 D-11 blocked_by-via-field-not-raise convention; verified via plan-spec inline smoke: 8000/65.40 -> 123 (ceil of 122.32...), 8000/80.00 -> 100 (exact division), 8000/0.00 -> None, 8000/-1.00 -> None; test_pnts_01_simple_breakeven_ceil_division flipped from Wave-0 strict-xfail to PASS with 4 inline assertions) |
| PNTS-02 | Phase 8 | Done (08-03 — lib.points.npv_breakeven(points_cost, monthly_savings, hold_months, discount_rate_annual) cumulative-discounted-sum walk per 08-RESEARCH §5.2 documented formula; two branches r>0 multiplicative discount factor + r==0 collapse to undiscounted accumulation per D-03-03; cross-validated three ways (numpy_financial.nper(0.07/12, 65.40, -8000) = 214.95 ceil=215 + closed-form n=-ln(1-8000*r/m)/ln(1+r) at r=0.07/12 = 214.95 + engine walk crosses zero at m=215); engine-actual values pinned (planner's '160 months at 7%' was mathematically wrong — see Plan 08-03 SUMMARY deviation #1); evaluate(req) discriminated-union dispatcher reports BOTH simple AND npv side-by-side per ROADMAP SC-4/D-04 with diverge=True iff both non-None AND unequal; decision='buy_points' iff cum_npv_at_hold>=0 with defensive force to skip_points when simple is None per D-03-04; test_pnts_02_npv_breakeven_decision_dispatcher flipped from Wave-0 strict-xfail to PASS with 2 scenarios [zero-discount simple==npv==123 + 7%-discount simple=123 npv=215 diverge=True]; D-03-07 NEW LOCKED DECISION via Rule-2 fix: relaxed 3 fields from Money(ge=0) to signed Decimal so the documented rate-up-scenario edge case round-trips through PointsResponse mirroring Phase 6 D-03 RefiCashflow.amount precedent) |

...
| 7. Estimated APR (Reg Z Appendix J) | 7/8 | In progress — 07-00..07-06 complete (see prior progress narratives for detail through Wave 5); Wave 6 references-doc: references/apr-reg-z.md ships at 523 lines (>=250 min) with the six required sections per D-28 LOCKED inheritance from references/arm-mechanics.md — §1 Unit-Period Model (12 CFR Part 1026 Appendix J U-equation + collapse-to-PV + multi-advance generalization + engine-surface pointers), §2 Day-Count Conventions (30/360 default + actual/365 + actual/actual; v1 cross-validation scope; D-18 'small differences' non-treatment), §3 Odd First Period Handling (§1026.17(c)(4) long-case f∈[0,1) per day-count formula table; engine surfaces _compute_odd_first_period_fraction + odd_first_period_days shortcut per D-15/D-17; negative-f short case engine-supported but v1-uncovered; f>=1 rejected at boundary per D-16 with 45-day negative-path fixture cite), §4 Worked Example (Reg Z Appendix J §(c)(1) SC-1 anchor walkthrough — \$5,000/36/\$166.07 → 12.00% regulatory; engine emits 0.119994 in 1 iter; \|diff\|=0.000006 within Decimal('0.00001'); D-25 rationale for regulatory-not-engine pinning), §5 Newton-Raphson Convergence (algorithm + npf.rate seed with NaN/range fallback per D-02; Decimal('0.00001') tolerance reconciliation 125x tighter than Reg Z §1026.22(a)(2) per RESEARCH §Finding 1; D-06 dual-criterion convergence; D-07/SC-3 50-iter cap with APRConvergenceError; Decimal-vs-float discipline + _decimal_pow ln/exp helper + sanity guard), §6 Citations Summary (11 URLs verified 2026-05-02 per D-30 — eCFR Appendix J + §1026.4/17/18/22 + CFPB compliance guide + FFIEC APRWIN out-of-scope per D-02 + HMDA Platform sole oracle per D-01 + numpy_financial.rate docs + bug #131 + cross-phase internal references; LOCKED DECISIONS cross-reference table mapping D-NN to in-doc sections per Phase 6 §8 idiom) + Citation Index appendix with URL × section × last-verified-date triplets per Phase 6 §Appendix verifier-table idiom; lib.apr.APRRequest.__doc__ extended from abbreviated 'unit-period model + day-count conventions' citation to explicit ROADMAP SC-5 / D-29 cite-from contract naming all 4 doc scopes (mirrors Phase 5 ARMTerms.__doc__ → references/arm-mechanics.md pattern); Wave-0 APR-08 stub test_references_apr_reg_z_doc_present_with_required_sections flipped from pytest.fail placeholder to real assertions on (a) doc exists at repo root, (b) all 6 required section headers present (## 1. Unit-Period Model ... ## 6. Citations Summary), (c) lib/apr.py contains literal 'references/apr-reg-z.md' string per D-29; 482 passed + 4 skipped + 2 xfailed full suite [was 481+4+3; +1 net pass per APR-08 stub flip; -1 xfail; only APR-04 Wave 7 stub remains xfail]; 12 of 13 Wave-0 stubs flipped; mypy --strict + ruff check + ruff format --check all clean on lib/apr.py + tests/test_apr.py; 0 deviations — plan executed exactly as written; SC-5 closed; APR-01 + APR-02 + APR-03 + APR-05 + APR-06 + APR-07 + APR-08 CLOSED (7/8); 1 plan remaining (07-07 HMDA Platform oracle fixtures closes APR-04) | - |

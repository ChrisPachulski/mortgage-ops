...
| 7. Estimated APR (Reg Z Appendix J) | 4/8 | In progress — 07-00 + 07-01 + 07-02 + 07-03 complete (Wave 0 Nyquist gate; Wave 1 Pydantic boundary; Wave 2 Newton-Raphson engine — see prior progress narrative for full detail; Wave 3 odd-first-period helpers: lib/apr.py 642->805 lines / +163 net adding _compute_odd_first_period_fraction(origination, first_payment, day_count) helper for Reg Z §1026.17(c)(4) + Appendix J §(b)(5)(iii) supporting 30/360 (f=(days-30)/30) + actual/365 (f=(days-365/12)/(365/12)) + actual/actual (f via dateutil.relativedelta(months=1) for month-edge accuracy) returning Decimal in [-1, 1) with ValueError surfaces for negative-period and f>=1 and unsupported-day_count + wired APRRequest.odd_first_period_days into solve_apr via in-engine model_copy(update={unit_period_fraction: f_odd}) of the first PaymentScheduleEntry per D-17 (engine surface unchanged - solve_apr already consumed unit_period_fraction correctly via Plan 07-02's _unit_period_equation/_derivative) + module docstring extended with D-15..D-18 LOCKED DECISIONS + Day-count conventions section enumerating the three formulas + 1 new passing test test_odd_first_period_15_days_increases_apr_above_nominal as engine smoke gate per Plan Deviation Rule 2 sign-flip detector ($200k @ 6.5%/30yr Wikipedia anchor + odd_first_period_days=15 -> APR=0.065002 strictly above 6.50% nominal as required); 5 of 13 stubs flipped to PASS); 470 passed + 4 skipped + 10 xfailed [was 469+4+10; +1 net pass]; mypy/ruff clean across 4 plans; SC-1 anchor verified empirically \$5000/36/\$166.07 -> 0.119994 in 1 iter within Decimal('0.00001') of 12.00%; APR-01 + APR-02 + APR-03 CLOSED [from Plan 07-02]; 4 plans remaining (07-04 CLI -> 07-05 fixtures + Reg Z anchor -> 07-06 references doc -> 07-07 HMDA Platform oracle fixtures); CONTEXT D-01 oracle pivot from FFIEC APRWIN to HMDA Platform code-based oracle locked but ROADMAP/REQUIREMENTS rewording pending per CONTEXT D-10; CONTEXT D-03/D-04 propose tightening day_count Literal to ['30/360'] only and advance_schedule list bounds — neither tightening changes a field NAME so deferrable to a future re-plan or 07-04 CLI plan revision; 8 deviations across 07-00/07-01/07-02/07-03 — 1 Rule-2 in 07-02 (catch _decimal_pow ValueError + bound-check next iterate to convert Newton-domain blow-ups (1+i)<=0 to clean APRConvergenceError signals - only semantically load-bearing deviation in 07-02) + 1 Rule-2 in 07-03 (plan-endorsed simplification: solve_apr's actual/actual day_count branch with odd_first_period_days>0 uses 30-day proxy unit_days because the request surface lacks origination/first_payment dates; advanced callers use _compute_odd_first_period_fraction with explicit dates per D-17) + 5 Rule-3 hygiene (unicode-minus -> ASCII in 07-02 _derivative docstring + 07-02 module docstring NotImplementedError removal + 07-02 ruff auto-fix import-organize + 07-03 ASCII 'x' for cartesian-product separator in module docstring + 07-03 ruff format auto-applied 2 reformats end-of-Task-2/Task-4) + 1 Rule-1 informational in 07-03 (must_haves.truths claim 'APR ≈ 6.523%' was a back-of-envelope forecast off ~100x in magnitude; engine produces 0.065002 verified via direct U-equation residual computation; engine math direction is correct - APR strictly above 6.50% as required by sign-flip detector; Wave 5 will pin exact value via HMDA Platform-validated fixture per CONTEXT D-09) | - |

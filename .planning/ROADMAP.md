...
| 6. Refinance NPV | 5/7 | In-progress — 06-00 Wave 0 + 06-01 Wave 1 + 06-02 Wave 2 + 06-03 Wave 3 + 06-04 Wave 4 complete; lib/refinance.py 1097 lines (6 Pydantic models + SC-4 sign-validator + 4 private helpers + cashflow builder + evaluate_rate_and_term body + after-tax tax-shield branch + evaluate_cash_out body + _compute_tax_shield_cashflows helper [D-09 + RUL-11] + isinstance-based public evaluate() dispatcher + warnings.catch_warnings/StaleReferenceWarning capture in BOTH paths + Oracle 1/2/3 pinned NPV via Decimal equality); scripts/refi_npv.py 253 lines (JSON-in/JSON-out CLI mirroring scripts/affordability.py per D-13/D-17/D-18/D-19; --help fast 25ms with literal SC-5 cite "see references/refi-npv.md" + "outflows negative, savings positive"; reuses Phase 5 scripts/_cli_helpers.find_json_float_loc + make_decimal_type_envelope verbatim; TypeAdapter(RefiRequest) discriminated-union routing; 6-key WR-02 envelope on stderr); engine + CLI surfaces both fully operational; 448 passed + 4 skipped + 14 xfailed full suite; mypy --strict + ruff clean; Wave 5 Plan 06-05 fixtures + 11 stub flips unblocked at CLI surface level (Oracle 1/2/3 round-trip subprocess invocations now operational) | - |

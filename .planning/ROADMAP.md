...
| 6. Refinance NPV | 6/7 | In-progress — 06-00 + 06-01 + 06-02 + 06-03 + 06-04 + 06-05 complete (Wave 0 test scaffold + Wave 1 Pydantic models + Wave 2 rate-and-term engine + Wave 3 cash-out + after-tax + Wave 4 CLI + Wave 5 fixtures + 11 fixture-driven stub flips); lib/refinance.py 1097 lines (engine fully operational); scripts/refi_npv.py 253 lines (CLI fully operational); 6 hand-calc fixtures at tests/fixtures/refinance/ (positive_npv_200bps_drop_2k_costs.json + negative_npv_short_horizon.json + cash_out_proceeds_50k.json + breakeven_divergence.json + sign_validator_outflow_positive.json + after_tax_mode_smoke.json — all expected values empirically derived via lib.refinance.evaluate per Phase 5 D-04 [REVISED] hand_calc_check witness pattern; D-03 RefiCashflow.kind citation coverage 5/5 via fixture-side cashflows_kinds arrays); 459 passed + 4 skipped + 3 xfailed full suite (+11 net pass from Wave 5 stub flips; only 2 Wave-6 doc stubs remain xfailed); SC-1 (positive + negative NPV with sign convention) + SC-2 (breakeven dual-form labeled with divergence anchor) + SC-3 (cash-out cash_proceeds + new_monthly_pi + total_interest_delta) + SC-4 (RefiCashflow sign-validator at model + CLI layers) all closed; REFI-01..03 + REFI-05..07 + REFI-08 requirements completed; Wave 6 (Plan 06-06 references/refi-npv.md doc body) is the final remaining plan — closes SC-5 verbatim phrase + REFI-09; pre-commit mypy + ruff clean; pre-existing mypy --strict 'Source file found twice' baseline issue logged in deferred-items.md per SCOPE BOUNDARY | - |

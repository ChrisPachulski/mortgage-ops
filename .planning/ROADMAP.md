...
- [ ] **Phase 8: Stress Tests & Points Breakeven** - Parameter sweeps + discount-points NPV (2/7 plans complete; Wave-0 Nyquist scaffold + Wave-1 Pydantic models shipped: 13 stress + 5 points strict-xfail stubs with 2 flipped to passing at the model-contract layer; ROADMAP SC-5 summary-before-rows + SC-4 simple-vs-NPV-side-by-side both pinned at the Pydantic model layer; lib/stress.py 257 lines + lib/points.py 140 lines created with discriminated-union request models + cross-plan evaluate() stubs for Plans 08-02/08-03; 504/4/17 baseline)
...
| 8. Stress Tests & Points Breakeven | 3/7 | In progress — Plans 08-00 + 08-01 + 08-02 complete (Wave 0 Nyquist gate + Wave 1 Pydantic v2 type contract + Wave 2 stress engine). Plan 08-02 (Wave 2): lib/stress.py 257->549 lines +245 net new with 4 public helpers + 1 private helper composing Phase 3 build_schedule / Phase 4 affordability.evaluate / Phase 5 build_arm_schedule per per-cell loops — rate_shock(loan, rates, baseline_label) re-solving PMT for each rate with delta_vs_baseline_monthly + delta_vs_baseline_pct + RATE_SHOCK_MONOTONE_PI invariant check (08-RESEARCH §6.4); income_shock(base_request, reductions, dti_threshold) per-applicant gross_monthly_income scaling per D-02-03 + Phase 4 D-06 sum aggregation; _synthesize_index_path covering closed-set RatePath dispatch per D-01-05 + D-02-04 (parallel-shift / gradual-rise / fall-then-rise) generating one IndexPathEntry per reset trigger via promoted lib.arm.compute_reset_triggers; arm_path(base_arm_request, paths) per-cell loop on lib.arm.build_arm_schedule capturing total_interest/max_payment/reset_count/highest_rate per row; evaluate(req) isinstance-dispatch over the discriminated union assembling StressResponse with SC-5 summary-before-rows declaration order (D-01-02). lib/arm.py 487->496 lines +9 net new for D-02-01 LOCKED public-API promotion of _compute_reset_triggers via single-line rename + backward-compat alias `_compute_reset_triggers = compute_reset_triggers` mirroring Phase 5 D-14 quantize_rate promotion exactly; in-module callsite in build_arm_schedule updated to public name for hygiene; Phase 5 baseline preserved verbatim — pytest tests/test_arm.py returns 36 passed + 1 xfailed unchanged. tests/test_stress.py 5 of 11 Wave-0 xfails flipped to synthesized in-process assertions (no fixtures yet — Plan 08-05 ships the 14 fixtures): rate-shock $400k/30yr at {0.06,0.065,0.07} pins Phase 3 oracle 2528.27 verbatim; income-shock single_applicant-shape baseline at 0% reduction matches baseline dti_back exactly + 50% reduction roughly doubles AND breaches 0.43 threshold; arm-path 5/1 ARM 30yr base + 3 canonical paths produce positive total_interest with parallel-shift > fall-then-rise sanity dominance + reset_count==25 across all 3 paths for [61,73,...,349] trigger sequence; sc5 monotone-pi invariant clean for monotone-increasing rates. Full suite 509 passed + 4 skipped + 12 xfailed (was 504+4+17 at Plan 08-01 close; +5 net pass / -5 xfailed exactly per plan; zero regression to Plan 08-01 baseline; remaining 12 xfailed = 4 Plan-08-04 CLI stubs + 1 Plan-08-04 envelope-uniformity + 1 Plan-08-05 SC-5 size-budget + 1 inherited Phase 5 ARM oracle + 5 Plan-08-03/08-05 points stubs). mypy --strict + ruff check + ruff format --check all clean across lib/stress.py + lib/arm.py + tests/test_stress.py. 3 deviations all auto-fixed inline (1 Rule-1/3 hygiene blend per-task incremental imports + PT018 split + max() Optional-narrowing key functions per Phase 7 Plan 07-01 idiom; 1 Rule-1 bug Loan(loan_type='conventional') not in Literal carried over from Plan 08-01 deviation #4 -> 'fixed'; 1 Rule-3 hygiene unused noqa: TC001 dropped on AffordabilityRequest after Task 3 promoted import to runtime use). D-02-01..D-02-06 LOCKED decisions honored verbatim. STRS-01 + STRS-02 + STRS-03 closed at the math layer (REQUIREMENTS.md updated to Done; STRS-04 CLI remains Pending pending Plan 08-04). Wave 3 (Plan 08-03 points engine) unblocked. Wave 4 (Plan 08-04 CLIs) unblocked at engine surface — scripts/stress_test.py calls lib.stress.evaluate after TypeAdapter(StressRequest).validate_json. Wave 5 (Plan 08-05 fixtures) unblocked at engine entrypoint level. Phase 5 reverse-coupling permanently closed (compute_reset_triggers public + alias preserved) | 2026-05-04 |

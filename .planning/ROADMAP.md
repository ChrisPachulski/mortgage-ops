...
| 7. Estimated APR (Reg Z Appendix J) | 2/8 | In progress — 07-00 + 07-01 complete (Wave 0 Nyquist gate: tests/conftest.py +21 lines apr_fixture loader + tests/test_apr.py 13 strict-xfail stubs covering APR-01..08 + 5 cross-cutting; Wave 1 Pydantic boundary: lib/apr.py 379 lines with D-01..D-08 LOCKED DECISIONS module docstring + 4 strict+frozen+forbid Pydantic v2 boundary models AdvanceScheduleEntry/PaymentScheduleEntry/APRRequest/APRResponse + APRRequest cross-field validators _advance_schedule_has_t0_advance for D-06 Reg Z Appendix J §(b)(2) + _payment_schedule_non_empty + APRResponse SC-4 anchor _summary_contains_literal_estimated_apr enforcing literal 'estimated APR' AND forbidding bare 'APR' regex \\bAPR\\b after stripping with 'APR tolerance' allow-listed + APRResponse iterations Field(ge=1, le=50) pinning ROADMAP SC-3 50-iteration cap at the model boundary D-07 + APRResponse tolerance_check dict[str, Any] | None D-08 + solve_apr stub raising NotImplementedError pointing at Wave 2 with full algorithm docstring; 1 Wave-0 stub flipped); 466 passed + 4 skipped + 13 xfailed [was 465+4+14; +1 net pass]; mypy/ruff clean; 2 Rule-3 hygiene deviations across 07-00 + 07-01 (per-task incremental import build + noqa TC001 for Pydantic-runtime-resolved Money matching lib/models.py noqa TC003 idiom); APR-01 partially closed at the model boundary (full closure when Wave 2 ships solver body); 6 plans remaining (07-02 Newton-Raphson engine → 07-07 oracle fixtures); CONTEXT D-01 oracle pivot from FFIEC APRWIN to HMDA Platform code-based oracle locked but ROADMAP/REQUIREMENTS rewording pending per CONTEXT D-10; CONTEXT D-03/D-04 propose tightening day_count Literal to ['30/360'] only and advance_schedule list bounds — neither tightening changes a field NAME so deferrable to a future re-plan or 07-04 CLI plan revision | - |

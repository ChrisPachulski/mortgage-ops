...
| 7. Estimated APR (Reg Z Appendix J) | 3/8 | In progress — 07-00 + 07-01 + 07-02 complete (Wave 0 Nyquist gate: tests/conftest.py apr_fixture loader + tests/test_apr.py 13 strict-xfail stubs; Wave 1 Pydantic boundary: lib/apr.py 379 lines with 4 strict+frozen+forbid boundary models + APRRequest cross-field validators + APRResponse SC-4 anchor + iterations Field(ge=1, le=50) D-07 + tolerance_check dict D-08 + solve_apr NotImplementedError stub; Wave 2 Newton-Raphson engine: lib/apr.py 642 lines / +263 net replacing solve_apr stub body with full Newton-Raphson iteration — 4 helpers (_decimal_pow via Decimal.exp(Decimal.ln) for fractional exponents D-13 + _unit_period_equation f(i) per Reg Z Appendix J §(b) + _derivative f'(i) closed-form validated against numerical derivative at relative-error 1e-12 + _seed_apr via npf.rate with NaN/inf/range fallback D-11) + APRConvergenceError(ValueError) D-12 + 3 module-level constants TOLERANCE=Decimal('0.00001') D-09/D-10 + DOLLAR_RESIDUAL=Decimal('0.01') D-10 + MAX_ITER=50 D-12 + Newton iteration under localcontext(MONEY_CONTEXT) prec=28 + D-10 dual-criterion convergence rate<=0.00001 AND residual<=0.01 + D-14 quantize_rate at end + tolerance_check populated when disclosed_apr supplied; 4 of 13 stubs flipped to PASS); 469 passed + 4 skipped + 10 xfailed [was 466+4+13; +3 net pass]; mypy/ruff clean; SC-1 anchor verified empirically \$5000/36/\$166.07 -> 0.119994 in 1 iter within Decimal('0.00001') of 12.00%; APR-01 + APR-02 + APR-03 CLOSED; 5 plans remaining (07-03 odd-first-period helpers -> 07-04 CLI -> 07-05 fixtures + Reg Z anchor -> 07-06 references doc -> 07-07 HMDA Platform oracle fixtures); CONTEXT D-01 oracle pivot from FFIEC APRWIN to HMDA Platform code-based oracle locked but ROADMAP/REQUIREMENTS rewording pending per CONTEXT D-10; CONTEXT D-03/D-04 propose tightening day_count Literal to ['30/360'] only and advance_schedule list bounds — neither tightening changes a field NAME so deferrable to a future re-plan or 07-04 CLI plan revision; 4 deviations across plans 07-00/07-01/07-02 — 1 Rule-2 in 07-02 (catch _decimal_pow ValueError + bound-check next iterate to convert Newton-domain blow-ups (1+i)<=0 to clean APRConvergenceError signals — only semantically load-bearing deviation across all 3 Phase 7 plans) + 3 Rule-3 hygiene (unicode-minus -> ASCII in _derivative docstring + module docstring NotImplementedError removal + ruff auto-fix import-organize) | - |

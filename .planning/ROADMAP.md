...
| 7. Estimated APR (Reg Z Appendix J) | 5/8 | In progress — 07-00 + 07-01 + 07-02 + 07-03 + 07-04 complete (Wave 0 Nyquist gate; Wave 1 Pydantic boundary; Wave 2 Newton-Raphson engine; Wave 3 odd-first-period helpers — see prior progress narrative for full detail through Wave 3; Wave 4 CLI: scripts/apr_reg_z.py 184 lines created mirroring scripts/arm_simulate.py byte-for-byte per Phase 5 D-07 + Phase 3 D-17/D-18/D-19 + Phase 4 D-13 + Phase 7 D-19/D-20/D-21/D-22 inheritance — argparse + --input only + sys.path injection AFTER args parser.parse_args + lazy-import block (lib.apr APRConvergenceError/APRRequest/solve_apr + pydantic VERSION/ValidationError + scripts._cli_helpers find_json_float_loc/make_decimal_type_envelope) so --help is fast (D-18; verified by sys.modules inspection inline subprocess test mirroring tests/test_arm.py pattern verbatim) + JSON-float pre-validation gate via Phase 5 factor-extracted helpers (zero new helpers added in Phase 7 — purely a consumer) + Pydantic ValidationError surfaces e.json() on stderr (6-key shape) + APRConvergenceError caught explicitly with 6-key envelope per D-21 (type='value_error', loc=['solver'], ctx={class:'APRConvergenceError', iterations, last_residual, last_i}) mirroring scripts/affordability.py:221-238 MissingCountyDataError BLOCKER-fix verbatim + happy path APRResponse.model_dump_json(indent=2) on stdout exit 0 / boundary failure exit 2 + epilog documents APRRequest input shape + APRResponse output shape + cites references/apr-reg-z.md per D-20 (Phase 5 ARM CLI does NOT cite arm-mechanics.md; Phase 7 sets cite-from-script-help precedent per ROADMAP SC-5) + literal 'estimated APR' phrase 4x in file (module docstring + epilog x3) per D-22 anchor #1 chain Wave 1 model boundary -> Wave 4 CLI surface -> Wave 4 regex test; tests/test_apr.py 5 Wave-0 xfail stubs flipped (test_apr_response_uses_literal_estimated_apr_text + test_apr_cli_subprocess_round_trip + test_apr_cli_help_does_not_import_lib_apr + test_apr_cli_rejects_float_loan_amount + test_apr_cli_error_envelope_uniformity); 10 of 13 stubs flipped to PASS); 475 passed + 4 skipped + 5 xfailed [was 470+4+10; +5 net pass exactly per the 5 stub flips, -5 xfail exactly corresponding to the flipped stubs]; mypy/ruff clean across 5 plans; SC-1 anchor verified empirically AND through the CLI subprocess surface — \$5000/36/\$166.07 -> 0.119994 in 1 iter within Decimal('0.00001') of 12.00%; APR-01 + APR-02 + APR-03 + APR-06 + APR-07 CLOSED (5/8); 3 plans remaining (07-05 fixtures + Reg Z anchor -> 07-06 references doc -> 07-07 HMDA Platform oracle fixtures); CONTEXT D-01 oracle pivot from FFIEC APRWIN to HMDA Platform code-based oracle locked but ROADMAP/REQUIREMENTS rewording pending per CONTEXT D-10; CONTEXT D-03/D-04 propose tightening day_count Literal to ['30/360'] only and advance_schedule list bounds — neither tightening changes a field NAME so deferrable to a future re-plan; 12 deviations across 07-00/07-01/07-02/07-03/07-04 — 1 Rule-2 in 07-02 (catch _decimal_pow ValueError + bound-check next iterate to convert Newton-domain blow-ups (1+i)<=0 to clean APRConvergenceError signals) + 1 Rule-2 in 07-03 (plan-endorsed simplification: solve_apr's actual/actual day_count branch with odd_first_period_days>0 uses 30-day proxy unit_days; advanced callers use _compute_odd_first_period_fraction with explicit dates per D-17) + 8 Rule-3 hygiene (07-02 unicode-minus -> ASCII in _derivative docstring + 07-02 module docstring NotImplementedError removal + 07-02 ruff auto-fix import-organize + 07-03 ASCII 'x' for cartesian-product separator + 07-03 ruff format auto-applied 2 reformats + 07-04 mypy --strict no-redef rename `envelope` to `convergence_envelope` + 07-04 ruff I001 import-organization auto-fix + 07-04 PT018 split + ruff format on tests/test_apr.py) + 1 Rule-1 informational in 07-03 (must_haves.truths claim 'APR ≈ 6.523%' was a back-of-envelope forecast off ~100x in magnitude; engine math direction correct; Wave 5 will pin exact value via HMDA Platform-validated fixture per CONTEXT D-09) + 1 Rule-1 bug-fix in 07-04 (envelope-uniformity test surface — Plan §Task 2 suggested 'missing advance_schedule' but Pydantic v2 emits 'missing' with only 5 keys (no ctx); routed through model_validator surface (value_error, always 6 keys with ctx) by using unit_period_offset=1 to violate _advance_schedule_has_t0_advance per D-06 — same fix pattern Phase 5 tests/test_arm.py::test_cli_error_envelope_uniformity discovered first) | - |

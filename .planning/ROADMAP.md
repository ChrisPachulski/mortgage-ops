...
| 8. Stress Tests & Points Breakeven | 2/7 | In progress — Plan 08-00 (Wave 0) shipped the Nyquist gate (13 stress + 5 points strict-xfail stubs + stress_fixture/points_fixture loaders + 3 .gitkeep placeholders); Plan 08-01 (Wave 1) ships the Pydantic v2 type contract — lib/stress.py 257 lines (8 BaseModel subclasses + StressRequest Annotated discriminated union by mode='rate-shock' \| 'income-shock' \| 'arm-reset' mirroring Phase 4 AffordabilityRequest verbatim per D-01-01 + StressResponse declaring `summary: ScenarioSummary` BEFORE `rows: list[StressRow]` pinning ROADMAP SC-5 verbatim closure at the model layer per D-01-02 + StressRow carrying summary scalars only [no full Schedule.payments[] arrays] per D-01-03 SC-5 100KB budget guard + RatePath.name closed-set Literal[parallel-shift, gradual-rise, fall-then-rise] per D-01-05 ROADMAP SC-3 verbatim + ScenarioSummary.stress_invariant_violations: list[str] = [] fail-loud-via-list per D-01-06 + evaluate() cross-plan stub for Plan 08-02) + lib/points.py 140 lines (PointsRequestFromSavings + PointsRequestFromLoans discriminated by mode + PointsResponse carrying both simple_breakeven_months: int\|None AND npv_breakeven_months: int\|None side-by-side per ROADMAP SC-4 verbatim per D-01-07 + discount_rate_annual REQUIRED with no default per D-01-04 fail-loud-on-implicit-default doctrine matching Phase 4 D-12 max_dti + Phase 5 D-02 floor_rate; Phase 6 cross-phase coupling deferred via additive non-breaking change documented in references/points-breakeven.md (Plan 08-06 lift); evaluate() cross-plan stub for Plan 08-03); tests/test_stress.py 2 Wave-0 xfails flipped (test_stress_request_discriminated_union_by_mode + test_sc5_summary_table_appears_before_rows_in_json). 504 passed + 4 skipped + 17 xfailed full suite (was 502+4+19; +2 net pass exactly per the 2 stub flips, -2 xfailed exactly corresponding to the flipped stubs; 0 failed; 0 errored; zero regression to Plan 08-00 baseline). 5 deviations [3 Rule-3 hygiene + 2 Rule-1 bug; all auto-fixed inline]: removed unnecessary `# type: ignore[empty-body]` from evaluate() stubs since body raises NotImplementedError; ruff RUF002 ambiguous-unicode `×` replaced with ASCII `x`; ruff TC001 false-positive on Pydantic-runtime-required imports suppressed via `noqa: TC001  # Pydantic resolves field annotations at runtime` per Phase 7 lib/apr.py:163 idiom; plan-spec test used Loan(loan_type='conventional') but Loan.loan_type Literal does not include 'conventional' [changed to 'fixed']; plan-spec test used adapter.validate_python({...with model_dump(mode='json') strings...}) but Pydantic v2 strict-mode rejects string->Decimal coercion in Python path [switched to validate_json(json.dumps(...)) per Phase 4 _request_from_fixture idiom]. D-01-01..D-01-07 LOCKED decisions honored verbatim. Wave 2 (Plan 08-02 stress engine) unblocked — replaces lib.stress.evaluate() body; Wave 3 (Plan 08-03 points engine) unblocked — replaces lib.points.evaluate() body. Plans 08-04..08-06 cascade-unblocked at the model surface | 2026-05-03 |

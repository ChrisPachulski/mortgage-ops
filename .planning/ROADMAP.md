...
- [ ] **Phase 8: Stress Tests & Points Breakeven** - Parameter sweeps + discount-points NPV (4/7 plans complete; Wave 0 Nyquist scaffold + Wave 1 Pydantic models + Wave 2 stress engine + Wave 3 points engine all shipped; STRS-01..03 + PNTS-01..02 closed at the math layer; ROADMAP SC-5 summary-before-rows + SC-4 simple-vs-NPV-side-by-side both pinned at the Pydantic model layer + engine layer; lib/stress.py 549 lines + lib/arm.py public-API promotion + lib/points.py 291 lines all complete; 511/4/10 baseline; remaining 10 xfailed = 8 Phase-8 (Plans 08-04/08-05) + 1 inherited Phase 5 ARM oracle + 1 nudge; CRITICAL pre-emptive flag for Plan 08-05 author: planner-claimed '160 months at 7% NPV breakeven' is mathematically wrong; engine produces 215 cross-validated against numpy_financial.nper + closed-form annuity + engine walk; Plan 08-05 SC-4 divergence-pin fixture must use 215 OR re-parameter)
...
| 8. Stress Tests & Points Breakeven | 4/7 | In progress — Plans 08-00 + 08-01 + 08-02 + 08-03 complete (Wave 0 Nyquist gate + Wave 1 Pydantic v2 type contract + Wave 2 stress engine + Wave 3 points engine). Plan 08-03 (Wave 3): lib/points.py 140->291 lines +151 net new with simple_breakeven [Decimal-safe ceil(points_cost/monthly_savings) via to_integral_value(rounding=ROUND_CEILING) under localcontext(MONEY_CONTEXT); returns int|None where None signals monthly_savings<=0 per D-03-01 mirroring Phase 4 D-11] + npv_breakeven [cumulative-discounted-sum walk per 08-RESEARCH §5.2; two branches r>0 multiplicative discount factor + r==0 collapse to undiscounted accumulation per D-03-03; cross-validated three ways against numpy_financial.nper + closed-form annuity + engine walk all converging on 215 months at 7%/$8000/$65.40/240mo] + _derive_monthly_savings [private helper for from_loans branch per D-03-05] + replaced Plan 08-01 evaluate() NotImplementedError stub with full discriminated-union dispatcher [isinstance-narrows; reports BOTH simple AND npv side-by-side per ROADMAP SC-4/D-04; diverge=True iff both non-None AND unequal; decision='buy_points' iff cum_npv_at_hold>=0 with defensive force to skip_points when simple is None per D-03-04; NEGATIVE_OR_ZERO_SAVINGS_<value> warning per D-03-01]. tests/test_points.py 74->128 lines +54 net new: 2 of 5 Wave-0 xfails flipped to engine-actual values cross-validated against numpy_financial.nper [test_pnts_01 4 inline assertions covering ceil case 123 + exact-division 100 + zero/negative None edges + test_pnts_02 2 scenarios (zero-discount simple==npv==123 + 7%-discount simple=123 npv=215 diverge=True; ENGINE-ACTUAL not planner-claimed 160)]. Full suite 511 passed + 4 skipped + 10 xfailed (was 509+4+12 at Plan 08-02 close; +2 net pass / -2 xfailed exactly per plan; zero regression to Plan 08-02 baseline; remaining 10 xfailed = 4 Plan-08-04 stress CLI + 1 Plan-08-04 stress envelope-uniformity + 1 Plan-08-05 SC-5 size-budget + 1 inherited Phase 5 ARM oracle + 2 Plan-08-04 points CLI + 1 Plan-08-05 SC-4 points divergence-pin). mypy --strict + ruff check + ruff format --check all clean. 4 deviations all auto-fixed inline: (1) Rule-1 plan-spec acceptance value '160 months at 7% discount' is mathematically incorrect (cross-validated three ways: numpy_financial.nper(0.07/12, 65.40, -8000)=214.95 ceil=215 + closed-form n=-ln(1-8000*r/m)/ln(1+r) at r=0.07/12=214.95 + engine walk crosses zero at m=215); planner's 08-RESEARCH §5.4 narrative cum_npv values internally inconsistent with §5.2 formula; engine ships §5.2 formula correctly + tests pin engine-actual 215; gap is 92 months not 37 — pre-emptive flag added to SC-4 xfail-stub docstring so Plan 08-05 author sees engine-actual value when authoring divergence-pin fixture. (2) Rule-2 Plan 08-01 type contract used Money(ge=0) on three fields that the engine legitimately produces negatives for in the documented rate-up edge case [PointsRequestFromSavings.monthly_savings + PointsResponse.monthly_savings + PointsResponse.cumulative_npv_at_hold]; relaxed to signed Decimal=Field(strict=True, max_digits=14, decimal_places=2) mirroring Phase 6 D-03 RefiCashflow.amount precedent verbatim; npv_breakeven signature also relaxed for documentation accuracy; fix is purely additive (every previously-valid construction still validates; only previously-blocked rate-up paths newly succeed); recorded as new locked decision D-03-07. (3) Rule-3 hygiene: PT018 compound-assert split (assert simple_m is not None and npv_m is not None -> 2 single-condition asserts) + ruff format auto-applied to multi-line Field(...) call after relaxation. (4) Rule-3 hygiene: removed extraneous backslash-escape in test docstring (\\\$ -> \$) to clear Python SyntaxWarning at collection time. D-03-01..D-03-06 LOCKED decisions honored verbatim + 1 NEW D-03-07 added via Rule-2 fix. PNTS-01 + PNTS-02 closed at the math layer (REQUIREMENTS.md updated to Done; PNTS-03 CLI remains Pending pending Plan 08-04). Wave 4 (Plan 08-04 CLIs) unblocked at engine surface — scripts/points_breakeven.py calls lib.points.evaluate after TypeAdapter(PointsRequest).validate_json. Wave 5 (Plan 08-05 fixtures) unblocked at engine entrypoint level for the 3 points fixtures — CRITICAL pre-emptive flag for 08-05 author re: 160 vs 215 SC-4 divergence-pin. Phase 6 deferred coupling unchanged (PointsRequest.discount_rate_annual REMAINS REQUIRED with no default per D-01-04 + D-03-06; additive non-breaking edit when Phase 6 lands) | 2026-05-04 |

...
| 7. Estimated APR (Reg Z Appendix J) | 6/8 | In progress — 07-00..07-05 complete (Wave 0 Nyquist gate + Wave 1 Pydantic boundary + Wave 2 Newton-Raphson engine + Wave 3 odd-first-period helpers + Wave 4 CLI; see prior progress narratives for full detail through Wave 4); Wave 5 tests-and-fixtures: 4 JSON hand-calc fixtures shipped under tests/fixtures/apr/ — regz_appendix_j_5000_36_166_07.json [SC-1 anchor; D-25 LOCKED regulatory value Decimal('0.120000'); engine emits 0.119994 in 1 iteration, diff 0.000006 within Decimal('0.00001') tolerance], regz_appendix_j_odd_first_period_15_days.json [D-24 engine-emitted 0.065002; corrects RESEARCH back-of-envelope ~6.523% forecast which Plan 07-03 SUMMARY had already flagged as ~100x off in magnitude — actual bump is +0.0002 pp not +0.023 pp; Wave 7 HMDA Platform cross-validation per CONTEXT D-09 is definitive precision check], regz_appendix_j_odd_first_period_45_days.json [D-26 NEGATIVE-path; f=45/30=1.5 violates D-16; engine raises ValueError with documented message_substring], regz_appendix_j_unit_period_monthly_regular.json [D-24 Wikipedia regular monthly sanity; engine = nominal 0.065000 exactly]; tests/test_apr.py flipped 2 Wave-0 stubs (test_apr_reg_z_appendix_j_worked_example_returns_12_percent [APR-05/SC-1] + test_newton_raphson_iterations_under_50_for_all_fixtures [SC-3 cross-cutting sweep over 3 fixtures]) + added 4 new tests (test_apr_hand_calc_fixtures_match_expected parametric over 2 anchors per D-27 + test_odd_first_period_too_long_raises sibling for the 45-day fixture + test_decimal_pow_fractional_exponent_correctness D-13 sanity guard pinning sqrt(2) to catch order-of-magnitude regressions before SC-1 anchor); 481 passed + 4 skipped + 3 xfailed [was 475+4+5; +6 net pass exactly per the 6 newly-passing tests, -2 xfail corresponding to the 2 Wave-0 stub flips]; 11 of 13 Wave-0 stubs flipped (only APR-04 [Wave 7] + APR-08 [Wave 6] stay xfail); mypy --strict + ruff check + ruff format --check all clean on tests/test_apr.py; SC-1 anchor end-to-end through fixture surface PASSES; APR-01 + APR-02 + APR-03 + APR-05 + APR-06 + APR-07 CLOSED (6/8); 2 plans remaining (07-06 references doc closes APR-08 → 07-07 HMDA Platform oracle fixtures closes APR-04); 3 deviations in 07-05 — 1 Rule-1 bug-fix in plan spec (Plan §Task 2 min/max range Decimal('0.065100')..Decimal('0.065500') was ~100x magnitude error; followed plan NOTE + executor important_notes to ship D-24 engine-emitted Decimal('0.065002') with documentary correction note) + 2 Rule-3 hygiene (ruff PT011 added match=re.escape(msg_substring) parameter to pytest.raises(ValueError) + ruff format auto-applied 1 reformat after stub flips landed) | - |

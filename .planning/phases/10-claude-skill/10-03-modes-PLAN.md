...
must_haves:
  truths:
    ...
    - "modes/_shared.md `## Save Report` step writes `reports/{NNN:03d}-{mode}-{YYYY-MM-DD}.md` then calls `node orchestration/db-write.mjs --insert-report --json {meta}` per D-13-02 + D-13-03 + D-13-04, with `_profile.md save_report: false` override per D-13-05"
...
**ADD `## Profile Loading` section (per D-PROF-04 fallback):**

```markdown
## Profile Loading

Before any mode runs, attempt to read `modes/_profile.md` (User Layer; gitignored). If the file exists, parse it as YAML and extract the four knobs: `verbosity`, `citation_density`, `save_report`, `disambiguation`.
...
After every mode invocation produces a report, write the report to disk and persist a row to DuckDB. This is unconditional unless the user opts out via `_profile.md` `save_report: false`.

Step 1 — Determine the next report sequence number:
   ```
   node orchestration/db-write.mjs --query "SELECT COUNT(*)+1 AS next_seq FROM reports"
   ```
   The handler returns `{"next_seq": <int>}` on stdout (Phase 9 cmdQuery pattern).
...
Step 3 — Write the report markdown body to that file path. The report content is the human-readable narration (this same response, or its formatted form per D-VOICE-01).

Step 4 — Persist via Phase 9 DuckDB handler per D-13-04. Construct the JSON metadata payload:
   ```json
   {
     "scenario_id": <int or null>,
     "kind": "<mode>",
     "markdown_blob": "<full file contents>",
     "filename": "reports/{NNN:03d}-{mode}-{YYYY-MM-DD}.md"
   }
   ```
   Then invoke:
   ```
   node orchestration/db-write.mjs --insert-report --json '<json-payload>'
   ```
   The withLock-gated INSERT is already shipped (Phase 9 D-03-04 / PERS-03).

Step 5 — Override per D-13-05: if `_profile.md` `save_report: false`, SKIP all of steps 1-4. Skill default per D-13-03 is unconditional save; this opt-out is the only escape hatch. Reports/ is gitignored (Phase 1 FND-08) so saved reports do not pollute git history.

This section closes SKLL-13 in Phase 10; tests `test_report_filename_format` + `test_report_persisted_to_duckdb` (Wave 0 stubs from Plan 10-00) flip in Wave 5 to enforce both the filename pattern and the DuckDB row.
```
...
- Contains the literal substring "node orchestration/db-write.mjs --insert-report" (D-13-04)
...
The file body MUST contain a YAML block with EXACTLY these four top-level keys (no others) per D-PROF-01:
...
Use this verbatim block (from CONTEXT.md D-PROF-01):
...
Wrap that block with a top-of-file markdown header + the copy instructions:

```markdown
<!-- Copy this file to .claude/skills/mortgage-ops/modes/_profile.md
     (without the .example) and edit. The .md file is User Layer per
     DATA_CONTRACT.md and gitignored — your edits stay private. -->

# Skill Behavior Profile (User Layer)

Four knobs that scale this skill's narration and default behavior. Per LOCKED DECISIONS D-PROF-01 and D-PROF-02, this file does NOT duplicate calc inputs (joint income, applicants, monthly debts, state_fips, county_fips, escrow, va block, target property value, lender preferences) — those live in `config/household.yml` + `config/profile.yml` per Phase 1 DATA_CONTRACT.

If `_profile.md` is missing on a fresh checkout, `_shared.md` falls back to the four defaults below (D-PROF-04: standard / inline / true / always-ask).
...
## Field semantics (CONTEXT.md D-PROF-01)

- **`verbosity`** scales narration length. `concise` = number + 1-line context; `standard` = full UI-SPEC three-part template (number / interpretation / citation); `verbose` = full citations + worked-example breakdowns + footnoted cross-references to references/*.md (D-VOICE-02).
- **`citation_density`** controls how aggressively regulatory references appear. `full` = every claim cited; `inline` = key claims; `minimal` = only blocking claims (e.g., DTI cap rejections).
- **`save_report`** is the ONLY user-level override of D-13-03's unconditional auto-write. Set `false` to suppress reports/{NNN}-{mode}-{YYYY-MM-DD}.md writes AND the matching DuckDB INSERT.
- **`disambiguation`** controls ambiguous-prompt behavior. `always-ask` (default) matches UI-SPEC §a printed disambiguation question; `auto-pick` silently routes to the most-likely mode (power-user opt-in).
```

CRITICAL — schema-correctness rules (D-PROF-01 + D-PROF-02):

1. The YAML body MUST parse cleanly with `yaml.safe_load`.
...
3. Calc inputs stay in `config/household.yml` + `config/profile.yml` per Phase 1 DATA_CONTRACT User Layer.
...
DO NOT create `modes/_profile.md` itself in this wave — User Layer enforcement (D-PROF-03 hook) prohibits the system from writing User Layer files.
  ...
  <verify>
    <automated>cd /Users/cujo253/Documents/mortgage-ops &amp;&amp; test -f .claude/skills/mortgage-ops/modes/_profile.example.md &amp;&amp; python -c "import yaml,re; t=open('.claude/skills/mortgage-ops/modes/_profile.example.md').read(); m=re.search(r'\`\`\`yaml\n(.*?)\n\`\`\`', t, re.DOTALL); body=m.group(1) if m else None; assert body, 'no yaml block'; d=yaml.safe_load(body); assert set(d.keys())=={'verbosity','citation_density','save_report','disambiguation'}, f'wrong keys: {set(d.keys())}'; print('OK 4 keys')" &amp;&amp; grep -q 'Copy this file' .claude/skills/mortgage-ops/modes/_profile.example.md &amp;&amp; test ! -f .claude/skills/mortgage-ops/modes/_profile.md</automated>
  </verify>
  <acceptance_criteria>
- `_profile.example.md` exists (≥ 25 lines, ≤ 60 lines — schema is intentionally narrow per D-PROF-02)
- File contains a fenced ```yaml block parseable by yaml.safe_load
...
- Top-of-file HTML comment explains how to copy the template
...
PART C — DATA_CONTRACT.md. Edit line 19 to correct the path. Current:
```
| `modes/_profile.md` | (Phase 10) user-specific narrative overrides for the Claude skill |
```
...
PART D — Verify enforcement works end-to-end. Try (and EXPECT FAILURE) staging the file:
```
mkdir -p .claude/skills/mortgage-ops/modes/
echo "test" > .claude/skills/mortgage-ops/modes/_profile.md
git add .claude/skills/mortgage-ops/modes/_profile.md  # should be no-op (gitignored)
git status --short  # should NOT show _profile.md
git add -f .claude/skills/mortgage-ops/modes/_profile.md  # force-add bypasses gitignore
python scripts/hooks/block-user-layer.py  # MUST exit non-zero with clear error
git restore --staged .claude/skills/mortgage-ops/modes/_profile.md
rm .claude/skills/mortgage-ops/modes/_profile.md
```

If the hook does NOT reject the force-add staging, the USER_LAYER_PATTERNS edit was wrong — re-investigate. The hook MUST refuse with a non-zero exit + a message naming the offending file.
  </action>
  <verify>
    <automated>cd /Users/cujo253/Documents/mortgage-ops &amp;&amp; grep -q '\.claude/skills/mortgage-ops/modes/_profile.md' .gitignore &amp;&amp; grep -q '\.claude/skills/mortgage-ops/modes/_profile.md' scripts/hooks/block-user-layer.py &amp;&amp; grep -q '\.claude/skills/mortgage-ops/modes/_profile.md' DATA_CONTRACT.md &amp;&amp; ! grep -E '^\| `modes/_profile.md`' DATA_CONTRACT.md</automated>
  </verify>
  <acceptance_criteria>
...
- End-to-end: `git add -f` of test _profile.md → hook rejection with clear error
  </acceptance_criteria>
  <done>
...
| T-10-40 | Tampering (D-13 Save Report missing) | _shared.md | mitigate | Task 1 acceptance grep-asserts the literal `node orchestration/db-write.mjs --insert-report` substring + filename convention; Wave 5 flips test_report_filename_format + test_report_persisted_to_duckdb |
...
- _shared.md contains `db-write.mjs --insert-report` (D-13-04) AND `save_report: false` (D-13-05) AND D-NUM-01..06 examples ($1,264.14 / 6.500% / 43.0% / bps)
